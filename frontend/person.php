...
<head>
    ...
    <title>Kweza Pay - Personal Account</title>
    ...
    <style>
        ...
        body.dashboard-person {
            background: #f4f7fb;
            position: relative;
            overflow-x: hidden;
        }

        body.dashboard-person::before,
        body.dashboard-person::after {
            content: "";
            position: fixed;
            inset: 0;
            pointer-events: none;
        ...
        /* Person dashboard spacing tweaks */
        .dashboard-person #balanceCard {
            margin-bottom: 10px;
        }

        @media (min-width: 901px) {
            .dashboard-person .pp-main.is-home {
                grid-template-columns: 480px 1fr;
            }

            .dashboard-person #recentActivityCard {
                width: 410px;
                max-width: 410px;
                justify-self: start;
            }

            .dashboard-person #dashboardActions {
                display: grid;
            ...
            .dashboard-person #dashboardActions .more-actions-item {
                grid-column: 1;
                grid-row: 2;
            }

            .dashboard-person #dashboardActions .kweza-save-action {
                grid-column: 2;
                grid-row: 2;
            }
        }

        .dashboard-person #homeScreen .pp-card {
            position: relative;
            overflow: hidden;
        }

        .dashboard-person #homeScreen .pp-card::before,
        .dashboard-person #homeScreen .pp-card::after {
            content: "";
            position: absolute;
            pointer-events: none;
            z-index: 0;
        ...
        .dashboard-person .pp-actions-grid {
            margin-bottom: 10px;
        }
    ...
    <link rel="stylesheet" href="assets/css/person_mobile.css?v=<?php echo time(); ?>">
</head>

<body class="dashboard-person">
    ...
    <header class="pp-header">
        <div class="pp-nav">
            ...
            <span id="nav-greeting" style="color: white; font-weight: 700; font-size: 18px; transition: all 10.0s ease; position: absolute; left: 50%; transform: translateX(-50%);">Personal Account</span>
        </div>
        
        <div class="pp-header-right-mobile">
        ...
        <div class="pp-header-actions" id="headerActions">
            ...
            <div onclick="switchScreen('me')" class="pp-header-icon mobile-only" data-label="My Profile" style="cursor: pointer;">
                <div class="profile-menu-icon" title="My Profile">
        ...
        <div id="homeScreen">
            ...
            <div class="pp-card" id="balanceCard">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="pp-balance-title">Total transaction history</span>
            ...
            <div class="pp-actions-grid" id="dashboardActions">
                ...
                <div class="pp-action-item more-actions-item" onclick="showMoreActions()">
                    <div class="pp-action-icon"><i class="fas fa-ellipsis-h"></i></div>
                    <span class="pp-action-label">More</span>
                </div>
    ...
    <!-- More Actions Modal -->
    <div id="moreActionsMenu" class="modal-overlay">
        <div class="modal-content">
            <h3 style="font-size: 18px; font-weight: 700; margin-bottom: 20px;">More Actions</h3>
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                <div class="pp-action-item" onclick="alert('Utility payments coming soon!')">
                    <div class="pp-action-icon" style="background: #f0f7ff; color: #11295E;"><i class="fas fa-lightbulb"></i></div>
                    <span class="pp-action-label">Pay Bills</span>
                </div>
                <div class="pp-action-item" onclick="alert('Airtime purchase coming soon!')">
                    <div class="pp-action-icon" style="background: #f0f7ff; color: #11295E;"><i class="fas fa-mobile-alt"></i></div>
                    <span class="pp-action-label">Buy Airtime</span>
                </div>
                <div class="pp-action-item" onclick="switchScreen('transactions'); closeModal('moreActionsMenu');">
                    <div class="pp-action-icon" style="background: #f0f7ff; color: #11295E;"><i class="fas fa-history"></i></div>
                    <span class="pp-action-label">History</span>
                </div>
            </div>
            <button class="btn btn-outline" style="margin-top: 25px;" onclick="closeModal('moreActionsMenu')">Close</button>
        </div>
    </div>
    ...
    <script>
        ...
        async function processPayment() {
            ...
            if (!mid || !amount) return alert('Please fill all fields');

            try {
                // Use relative path but ensure it's correct from /frontend/
                const res = await fetch('../backend/api/pay_merchant.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ merchant_token: mid, amount })
                });
        ...
        function showReceipt(data) {
            ...
            let html = `
                ...
                <div class="receipt-details-grid">
                    <div class="receipt-detail-item">Description: ${data.description || 'Kweza Transaction'}</div>
                    <div class="receipt-detail-item">Amount: MK ${parseFloat(data.amount).toLocaleString()}</div>
                    <div class="receipt-detail-item">Service Fee: (Direct via Provider)</div>
                    <div class="receipt-detail-item">Recipient: ${data.merchantName}</div>
                </div>
        ...
        async function downloadReceipt() {
            ...
            const refHeader = node.querySelector('.receipt-info-line:last-of-type');
            const ref = refHeader ? refHeader.innerText.split(': ')[1] : 'receipt';
        ...
        function viewTransactionReceipt(txn) {
            const isDebit = txn.sender_id == <?php echo $user['user_id']; ?>;
            const receiptData = {
                type: txn.txn_type === 'P2P' ? 'P2P Transfer' : 'Merchant Payment',
                merchantName: txn.description || (isDebit ? 'Transfer Out' : 'Transfer In'),
                amount: txn.amount,
                reference: txn.reference_code,
                date: new Date(txn.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
        ...
        window.addEventListener('load', function() {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    greeting.textContent = 'Personal Account';
